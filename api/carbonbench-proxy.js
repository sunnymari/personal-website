/**
 * Proxy Carbonbench with a curated snapshot fallback when their Railway DB is down.
 * Hit via rewrite: /api/carbonbench/:path* → /api/carbonbench-proxy?path=:path*
 */
...
function snapshotRecommend(model = "llama") {
...
function pathFromReq(req) {
  ...
  if (qPath) return qPath.replace(/^\/+/, "");
  const m = url.pathname.match(/\/api\/carbonbench(?:-proxy)?\/?(.*)$/);
  return (m?.[1] || "").replace(/^\/+/, "");
}
...
export default async function handler(req, res) {
  ...
  const url = new URL(req.url, "http://localhost");
  const path = pathFromReq(req) || "recommend";
  // Keep original query string but drop our rewrite `path` param
  const params = new URLSearchParams(url.search);
  params.delete("path");
  const search = params.toString() ? `?${params}` : "";

  try {
    const remote = await upstream(path, search);
    if (remote.ok && !remote.data?.error) {
      json(
        res,
        200,
        {
          ...remote.data,
          meta: { ...(remote.data.meta || {}), source: "carbonbench", carbonbenchOnline: true },
        },
        { "X-Sprout-Carbonbench": "live" },
      );
      return;
    }
  } catch {
  ...
  if (path.startsWith("recommend")) {
    const model = url.searchParams.get("model") || "llama";
    json(res, 200, snapshotRecommend(model), { "X-Sprout-Carbonbench": "snapshot" });
    return;
  }

  if (path === "carbon/all" || path.startsWith("carbon/all")) {
    json(res, 200, snapshotCarbonAll(), { "X-Sprout-Carbonbench": "snapshot" });
    return;
  }
  ...
  if (path === "health") {
    json(
      ...
      { "X-Sprout-Carbonbench": "snapshot" },
    );
    return;
  }

  json(res, 502, {
    error: "Carbonbench upstream unavailable and no snapshot for this path.",
    path,
  });
}
